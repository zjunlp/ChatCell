...
def main(data_filepath: Path, output_dir: Path):
    ...
    adata.var["feature_name"]=adata.var["features"].copy().str.upper()
    adata.var["feature_name"] = adata.var["feature_name"].astype(str)
    duplicates = adata.var["feature_name"].duplicated(keep=False)
    adata.var.loc[duplicates, "feature_name"] = (
        adata.var.loc[duplicates, "feature_name"] 
        + '_' 
        + adata.var.loc[duplicates, "feature_name"].groupby(adata.var["feature_name"]).cumcount().add(1).astype(str)
    )
